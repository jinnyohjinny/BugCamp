...
$(ALL_LABS): ## Build and run the specified lab
	...
	@if [ ! -d "labs/level-01/$@" ] && [ ! -d "labs/level-02/$@" ] && [ ! -d "labs/level-03/$@" ] && [ ! -d "labs/level-00/$@" ] && [ ! -d "labs/level-04/$@" ] && [ ! -d "labs/level-05/$@" ] && [ ! -d "labs/level-06/$@" ] && [ ! -d "labs/level-07/$@" ] && [ ! -d "labs/level-08/$@" ]; then \
		echo -e "$(RED)Error: Lab directory not found in labs/level-01/, labs/level-02/, labs/level-03/, labs/level-00/, labs/level-04/, labs/level-05/, labs/level-06/, labs/level-07/, or labs/level-08/$(NC)"; \
		exit 1; \
	fi
	...
	# Build and run the lab (try level-01 first, then level-02, then level-03, then level-00, then level-04, then level-05, then level-06, then level-07, then level-08)
	...
	elif [ -d "labs/level-05/$@" ]; then \
		cd labs/level-05/$@ && docker-compose up -d --build; \
	elif [ -d "labs/level-06/$@" ]; then \
		cd labs/level-06/$@ && docker-compose up -d --build; \
	elif [ -d "labs/level-07/$@" ]; then \
		cd labs/level-07/$@ && docker-compose up -d --build; \
	elif [ -d "labs/level-08/$@" ]; then \
		cd labs/level-08/$@ && docker-compose up -d --build; \
	else \
